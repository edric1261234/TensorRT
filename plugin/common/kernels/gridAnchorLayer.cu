...
template <unsigned nthdsPerCTA>
    ...
    template <unsigned nthdsPerCTA>
        __launch_bounds__(nthdsPerCTA)
        __global__ void multiScaleAnchorKernel(
                const MultiScaleAnchorParameters param,
                const int numAspectRatios,
                reduced_divisor divObj,
                const float* widths,
                const float* heights,
    ...
    pluginStatus_t multiScaleGridInference(
            cudaStream_t stream,
            const MultiScaleAnchorParameters param,
            const int numAspectRatios,
            const void* widths,
            ...
            )
    {
        ...
        if (dim > 5120)
        {
            const int BS = 128;
            const int GS = (dim + BS - 1) / BS;
            multiScaleAnchorKernel<BS><<<GS, BS, 0, stream>>>(param, numAspectRatios, divObj,
                    (const float*) widths, (const float*) heights,
                    (float*) outputData);
